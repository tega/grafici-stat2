...
ai rispettivi momenti teorici $\mu_{r}$, il secondo quando abbiamo
...
P_{H_{0}}(\overline{X}\geqq-0.536) & = & P_{H_{0}}(\overline{X}+0.545\geqq0.009)\\
 & = & P_{H_{0}}(\frac{\sqrt{5}\left(\overline{X}+0.545\right)}{0.008}\geqq2.515)\\
 & = & 0.59\%.\end{eqnarray*}
...
P_{H_{0}}(\overline{X}\leqq0.5985,\:\overline{X}\geqslant0.6015) & = & P_{H_{0}}(|\overline{X}-0.6|\geqq0.0015)\\
 & = & P_{H_{0}}(\frac{|\overline{X}-0.6|}{0.001}\geqq1.5)=13.4\%.\end{eqnarray*}
...
P\left(Z\leq q_{0.95}\right) & = & 0.95\end{eqnarray*}
...
\overline{x}\geqq k_{5\%}=-0.539\label{disequazione test destro}\end{equation}
...
$(X_{1},...,X_{n})$ per cui $S\geqq k_{\alpha}$ (e quindi tali per
cui il test è rifiutato) è chiamato la regione critica del test o
semplicemente regione critica\[
R=\left\{ (x_{1},...,x_{n})\in\Omega^{n}\mid S(x_{1},\ldots,x_{n})\geqq k_{\alpha}\right\} .\]
...
P(\min(X_{1},...,X_{5})\geq s)=P(X_{1}\geq s,X_{2}\geq s,...,X_{5}\geq s)\text{.}\]
 Utilizzando l'indipendenza degli $X_{i}$ \[
P(X_{1}\geq s,...,X_{5}\geq s)=\prod_{i=1}^{5}P(X_{i}\geq s).\]
 Infine, poiché $P(X_{i}\geq s)=1-P(X_{i}\leq s)$ (la distribuzione
di $X_{i}$ è continua), otteniamo la seguente identità \[
P(\min(X_{1},...,X_{5})\geq s)=P(X_{1}\geq s,...,X_{5}\geq s)=\prod_{i=1}^{5}\left(1-P(X_{i}\leq s)\right)\]
...
se \[
S(x_{1},...,x_{n})>k\]
 rifiutiamo $H_{0}$ altrimenti non la rifiutiamo (nel caso di un
test unilaterale destro). Il valore di $k$ che definisce la soglia
fra rifiuto e non rifiuto di $H_{0}$ è stato fissato in modo tale
che la probabilità di osservare valori più estremi di $k$ sia uguale
al livello di significatività $\alpha$ prescelto. Per tale motivo
si è soliti aggiungere al piede di $k$, $k_{d}$ e $k_{s}$ il valore
$\alpha$.
...
specie. Per fare ciò analizziamo il seguente esempio.
...
di densità di $X$ sotto le due ipotesi è riportato nella Figura.
%
\begin{figure}
\begin{centering}
\includegraphics[scale=0.4]{codiceR/grafici/standardNormalDensityWith095Quantile}
\par\end{centering}

\caption{Distribuzione Normale standard}



\end{figure}
Poiché l'ipotesi alternativa è situata alla destra di $H_{0}$, la
soglia $k$ della regione critica al livello di significatività desiderato
è definita da\[
P_{H_{0}}(X\geq k)=0.05\Rightarrow k_{5\%}=z_{95\%}=1.645.\]
 La regione critica%
...
quindi con l'intervallo $(k,\infty)$. Tuttavia è questo un caso più
unico che raro che mai si verifica nelle applicazioni reali.%
} è \[
