...
%
\begin{figure}[H]
 \centering{}\includegraphics[scale=0.4]{codiceR/grafici/funzioneRipartizioneUniformeDiscreta}\caption{Distribuzione uniforme discreta}
...
%
\begin{figure}[H]
 \centering{}\includegraphics[scale=0.4]{codiceR/grafici/funzioneRipartizioneUniformeContinua}\caption{Distribuzione uniforme continua}
...
%
\begin{figure}
\includegraphics[scale=0.4]{codiceR/grafici/numeroScarpeUomoDonnaFreqRelative}\includegraphics[scale=0.4]{codiceR/grafici/numeroScarpeFreqRelative}\caption{Frequenze relative popolazione studenti Statistica II - 2009}
...
\begin{figure}
\begin{centering}
\includegraphics[scale=0.4]{codiceR/grafici/confrontoDueDensitaNormali}\caption{Due funzioni di densità della famiglia parametrica Normale}

\par\end{centering}

\centering{}\label{Flo:graficoConfrontoDueNormali} 
\end{figure}


\noindent Nella Figura \ref{Flo:graficoConfrontoDueNormali} la formula
...
statistico può quindi essere rappresentato dalla tripla\begin{equation}
...
(indichiamo con $g$ la funzione di densità di $Q$)\begin{eqnarray*}
\mathcal{P} & = & \left\{ g(x_{1},x_{2},x_{3};\lambda)=f\left(x_{1};\lambda\right)\ f\left(x_{2};\lambda\right)\ f\left(x_{3};\lambda\right):\ \lambda>0\right\} \\
 & = & \left\{ g(x_{1},x_{2},x_{3};\lambda)=\lambda^{3}e^{-\lambda(x_{1}+x_{2}+x_{3})}I_{(0,\infty)}(x_{1})I_{(0,\infty)}(x_{2})I_{(0,\infty)}(x_{3}):\ \lambda>0\right\} .\end{eqnarray*}
 \end{esempio}
...
di un volo di linea sia una variabile aleatoria. Alla compagnia probabilmente
interessa soprattutto sapere i costi che un ritardo può generare in
funzione della sua gravità piuttosto che l'ammontare in minuti del
ritardo. I costi saranno quindi una certa funzione dei minuti di ritardo.
Generalizzando al caso astratto, introduciamo la seguente definizione.
...
\begin{esempio}Consideriamo l'esperimento aleatorio di lanciare una
volta un dado e di fare una scommessa sul possibile esito. Supponiamo
che la vincita o la perdita in franchi, notata $X$, dipenda dall'esito
dell'esperimento e sia descritta dalla funzione $X(\omega)=\omega-2$
dove $\omega$ rappresenta l'esito del lancio. Siamo interessati all'evento
\foreignlanguage{english}{$E'=[3,\infty)$} di guadagnare almeno 3
franchi. Il nostro esperimento è formalizzato da $\Omega=\{1,2,3,4,5,6\}$
...
del dado risulterà in un numero superiore o alla peggio uguale a 5.
L'evento $E$ nel nostro caso è dato dal sottoinsieme $\{5,6\}$ di
$\Omega$. La probabilità di vincere almeno 3 franchi è dunque uguale
alla probabilità che esca un cinque o un sei. Scriveremo\begin{equation}
P_{X}(E'):=P(X^{-1}(E'))=P(E)=\frac{1}{3}.\label{eq: probabilitaImmagini}\end{equation}
...
singolo lancio utilizzeremo la convenzione di indicare con $t$ l'esito
testa e con $c$ l'esito croce. Modelliamo questo esperimento con
lo spazio campionario $\Omega^{3}$ contenente gli 8 possibili esiti
\[
\begin{array}{cccc}
\omega_{1}=(t,t,t) & \omega_{2}=(c,t,t) & \omega_{3}=(c,t,c) & \omega_{4}=(t,t,c)\\
\omega_{5}=(t,c,c) & \omega_{6}=(c,t,c) & \omega_{7}=(t,c,c) & \omega_{8}=(c,c,c)\end{array}.\]
...
Q(\left\{ \omega_{i}\right\} )=p^{\#c}(1-p)^{\#t}\]
 dove {}``$\#c$\textquotedblright{}\ indica il numero di {}``$c$\textquotedblright{}\ in
$\omega_{i}$. La tripla $(\Omega^{3},\overset{n}{\underset{i=1}{\otimes}}\mathcal{E}_{i},Q)$
...
contare il numero di {}``$c$\textquotedblright{} presenti in $\omega$
...
X(\omega_{5})=1 & ,\: X(\omega_{6})=1 & ,\: X(\omega_{7})=1 & ,\: X(\omega_{8})=\frac{3}{2}\end{array}.\]
 La $V.A.$ $X$ assumerà così i valori $0,\frac{1}{2},1,\frac{3}{2}$.
Con che probabilità la variabile aleatoria $X$ sarà uguale a $0$?
Guardando la tabella dei possibili valori deduciamo che $X=0$ se
e solo se $\omega=\omega_{1}$, quindi \[
P_{X}(\left\{ 0\right\} )=P(X=0)=P(\left\{ \omega_{1}\right\} ).\]
In maniera analoga, dato l'evento $E'=[\frac{1}{2},1]\subset\mathbb{R}$
diremo che $E'$ si è realizzato se $X(\omega)\in E'$, dove $\omega$
denota la realizzazione su $\Omega$. La probabilità che $E'$ si
realizzi è uguale a \[
P_{X}(E')=P(X^{-1}(E'))=P(\left\{ \omega_{2},\omega_{3},\omega_{4},\omega_{5},\omega_{6},\omega_{7}\right\} ).\]
La probabilità dell'evento $E'=\{0,\frac{3}{2}\}$ è uguale a $P(\{\omega_{1},\omega_{8}\})$
e la probabilità di $P_{X}\left([0,1)\right)$ è semplicemente uguale
...
$Bin(3,p)$ con supporto $\{0,\frac{1}{2},1,\frac{3}{2}\}$ anziché
$\{0,1,2,3\}$.
...
estratto da $X$ induce uno spazio prodotto notato \begin{equation}
...
\mathcal{P}=\left\{ g(x_{1},...,x_{n};\theta)=\overset{n}{\underset{i=1}{\Pi}}f_{X_{i}}\left(x_{i};\theta\right):\ \theta\in\Theta\right\} .\]


\noindent Come in precedenza la densità di $Q_{X_{1,...,}X_{n}}$
è il prodotto delle $n$ funzioni di densità $f_{X_{i}}(\cdot;\theta)$.
...
lista $(x_{1},...,x_{n})$ che chiameremo vettore. La notazione maiuscola
denota la variabile aleatoria (funzione!) mentre la notazione minuscola
ne indica il valore realizzato. Ad esempio $x_{1}=X_{1}(\omega)$.\end{remark}
...
dove $E$ corrisponde agli indici delle schede con automobili di colore
nero. Il numero di esiti in $E$ è $N_{1}$. Come calcolato precedentemente
la probabilità di ciascun esito è $\frac{1}{N}$ e quindi $P(E)=\frac{N_{1}}{N}=0.45$.
La situazione è rappresentata graficamente nella Figura \ref{Flo:graficoColoreNero}.%
\begin{figure}
\begin{centering}
...
F_{X}(x) & =P_{X}\left((-\infty,x]\right)\nonumber \\
 ...
 & =\frac{\#\text{ unità con caratteristica }\leq x}{\#\text{ unità della popolazione}}\label{eq:ugualeAllaRipartizione}\end{align}
...
numero è pari a $1/N$ vale\begin{equation}
Q\left(\omega\right)=\frac{1}{N^{n}}\ \text{per ogni }\omega\in\Omega^{n}\ .\label{eq:probEsitoConReimmissione}\end{equation}
...
\text{2. Associo la cilindrata della }\omega(i)-\text{esima automobile.}\end{array}\right.\]
...
un $n$-campione di un esperimento statistico $(\mathbb{R},\mathcal{E},P_{X})$.
...
probabilità vista nel caso di un'estrazione con reinserimento (confronta
la \ref{eq:probEsitoConReimmissione}).\vspace{0.2cm}
...
\text{2. Associo la cilindrata dell'}\omega(i)-\text{esima automobile.}\end{array}\right.\]
...
Facciamo un esempio prendendo quale popolazione obiettivo il colore
delle automobili immatricolate. Supponiamo che nella popolazione obiettivo
ci siano $k$ automobili rosse. Calcoliamo \[
...
costruite tramite selezione senza reinserimento. Sebbene utili ai
fini dell'inferenza statistica, esse non sono $i.i.d.$ e non costituiscono
un $n$-campione di un esperimento statistico $(\mathbb{R},\mathcal{E},\mathcal{P})$.
La tripla $(\mathbb{R}^{n},\overset{n}{\underset{i=1}{\otimes}}\mathcal{E}_{i},Q_{X_{1},...,X_{n}})$
è uno spazio di probabilità ma non costituisce un esperimento statistico.
...
atteso di $X$ sarà pertanto uguale a \begin{eqnarray*}
...
in maniera simile al valore atteso quando noi {}``fissiamo'' uno
dei due argomenti.\vspace{0.2cm}
...
l'argomento fisso.\begin{eqnarray*}
...
dove $K$ è il numero dei diversi volori di $X$ osservabili nella
popolazione e \foreignlanguage{english}{$p_{k}$} rappresenta la frazione
di unità della popolazione (la frequenza dunque) aventi un valore
della caratteristica uguale a $x_{k}$. Sappiamo che la popolazione
è costituita da $N$ unità. Denotiamo con $a_{j}$ il valore della
caratteristica studiata della $j$-esima unità. Ad esempio, se decidessimo
di studiare la distribuzione del reddito della popolazione svizzera,
$a_{j}$ rappresenterebbe il reddito dello $j$-esimo individuo alla
data in cui è eseguito lo studio. Il parametro sconosciuto da stimare
è dunque uguale a \[
...
ma solo il primo momento e la varianza, per questa volta eviteremo
di specificare una famiglia parametrica di distribuzioni.
...
è una variabile aleatoria la cui distribuzione dipende indirettamente
dalla distribuzione della popolazione campione. Purtroppo vale la
seguente osservazione. Anche qualora la distribuzione della popolazione
obiettivo fosse conosciuta, la distribuzione della media $\overline{X}_{n}$
non sarebbe, in generale, derivabile analiticamente. Ci sono però
delle eccezioni. Una fra queste è la distribuzione Normale. Infatti,
quando le variabili aleatorie $X_{1},...,X_{n}$ sono $i.i.d.$ e
distribuite secondo la legge Normale $N(\mu,\sigma^{2})$, allora
anche la loro media è una variabile aleatoria Normale.
...
$U_{21}$ e $U_{35}.$
\item \noindent Considerate gli esiti $\omega_{1}=(1,2,5)$, $\omega_{2}=(5,2,1)$
e $\omega_{3}=(1,1,1)$ e calcolate per ciascuna delle tre variabili
$U$ il valore che esse assumono in $\omega_{i}$, $i=1,2,3.$
\item Mostrate che $X_{1}=$$U_{11}$+$U_{12}$+$U_{13}$ per $\omega_{1}$
e $\omega_{2}$. Vale anche per $\omega_{3}$?
\end{enumerate}
...
\noindent Il numero degli esiti favorevoli è uguale al numero di esiti
...
Per tale motivo otteniamo quale risultato finale\begin{equation}
...
Cov(T_{j},T_{s}) & = & -\frac{1}{k^{2}}.\end{eqnarray*}
...
E\left(\overline{X}_{n}\right) & = & E\left(\frac{1}{n}\sum_{j=1}^{k}g_{j}T_{j}\right)=\frac{1}{n}\sum_{j=1}^{k}g_{j}E\left(T_{j}\right)=\frac{1}{n}\frac{1}{k}\sum_{j=1}^{k}g_{j}\\
 & = & \frac{1}{N}\underset{\text{somma sull'intera pop.}}{\underbrace{\sum_{j=1}^{k}\sum_{i=1}^{n}a_{ji}}}=\mu\text{.}\end{eqnarray*}
...
V(\overline{X}_{n}) & = & \frac{1}{n^{2}}V(\sum_{j=1}^{k}g_{j}T_{j})=\frac{1}{n^{2}}\sum_{j=1}^{k}\sum_{s=1}^{k}g_{j}g_{s}Cov(T_{j},T_{s})\\
 ...
 & = & \frac{1}{n^{2}}\left(\frac{1}{k}\sum_{j=1}^{k}g_{j}^{2}-\frac{1}{k^{2}}\sum_{j=1}^{k}\sum_{s=1}^{k}g_{j}g_{s}\right).\end{eqnarray*}
...
e (\ref{campione stratificato proporzionale}) otteniamo \begin{eqnarray*}
n_{h} & = & cN_{h}\ \ \ \forall h\\
...
\begin{equation}
\sigma^{2}=\sum_{h=1}^{H}\pi_{h}\sigma_{h}^{2}+\sum_{h=1}^{H}\pi_{h}\left(\mu_{h}-\mu\right)^{2}\label{eq:a}\end{equation}
 \end{theorem}
...
dei due stimatori notiamo che\begin{eqnarray*}
...
V(\overline{X}_{n})-V(\overline{X}_{str,n}) & \simeq & \frac{1-f}{n}\left(\sigma^{2}-\sum_{h=1}^{H}\pi_{h}\sigma_{h}^{2}\right)\\
 & \underset{(\ref{eq:a})}{=} & \frac{1-f}{n}\sum_{h=1}^{H}\pi_{h}\left(\mu_{h}-\mu\right)^{2}\geq0\text{.}\end{eqnarray*}
...
di persone sarà compreso con \emph{{}``}molta probabilità'' fra
le $900$ e le $1^{\prime}100$ persone. Un secondo esempio potrebbe
...
$\overline{X}_{n}$ saranno molto concentrate attorno a $\mu$.
...
dopo la standardizzazione le variabili aleatorie $Y_{n}$ saranno
...
che $Y_{n}\sim(0,1)$. Cosa si potrà affermare sulla distribuzione
di $Y_{n}$ in questo caso? Il TLC è la soluzione a questa domanda.
...
è immediato verificare che \begin{equation}
P(|Z|\leq1.96)=0.95\ \text{.}\label{equazione livello confidenza}\end{equation}
...
P(\mu-1.96\sqrt{V(\overline{X}_{n})}\leq\overline{X}_{n}\leq\mu+1.96\sqrt{V(\overline{X}_{n})})=0.95.\label{intervallo interpretazione 1}\end{equation}
...
P(\overline{X}_{n}-1.96\sqrt{V(\overline{X}_{n})}\leq\mu\leq\overline{X}_{n}+1.96\sqrt{V(\overline{X}_{n})})=0.95.\label{intervallo interpretazione 2}\end{equation}
...
\noindent Osserviamo la sottile differenza fra la (\ref{intervallo interpretazione 1b})
...
Fissato dunque il livello $\alpha$ desiderato, occorre risolvere
l'equazione \[
P(|Z|\leq c)=\alpha\]
in funzione di $c$. Le soluzioni per i tre livelli $\alpha$ di significatività
...
di confidenza al variare del livello di significatività $\alpha$.\end{esercizio}
...
ma soggetta a continua verifica. Con l'arrivo di ulteriori informazioni
...
cioè esiste un $\theta\in\Theta$ tale per cui\ $f(x;\theta)$ è
la densità di $X$; 
...
$V.A.$) da usare quali stimatori del parametro sconosciuto $\theta$.
...
E(Y)=E\left(X^{r}\right)=\int_{-\infty}^{\infty}x^{r}f(x;\theta)dx\text{.}\label{momento r}\end{equation}
...
La dimostrazione della precedente uguaglianza segue direttamente dal
fatto che \[
...
M_{3}=\sum_{x=0}^{\infty}x^{3}\frac{\theta^{x}e^{-\theta}}{x!}=\theta+3\theta^{2}+\theta^{3}\text{.}\end{equation}
 Come potete ancora una volta constatare sia $M_{1}$ che $M_{2}$
sono particolari funzioni del parametro $\theta$ (confronta Figura
\ref{fig:Primo-e-secondo poisson}). %
\begin{figure}
...
\lim_{n\rightarrow\infty}P(|M_{r,n}-M_{r}|>\varepsilon)=0\text{.}\label{convergenza probabilita campione}\end{equation}
 \end{theorem}
...
grande \begin{equation}
M_{r,n}\simeq M_{r}(\theta_{0})\text{.}\label{approx momento}\end{equation}
...
uguale $1$. Scriveremo \begin{equation}
m_{3,n}=1\simeq M_{3}(\theta_{0})=\theta_{0}+3\theta_{0}^{2}+\theta_{0}^{3}.\label{nuova equazione secondo momento 0}\end{equation}
...
momento utilizzassimo il secondo momento, otterremmo la seguente relazione\begin{equation}
M_{2,n}\simeq M_{2}(\theta_{0})=\theta_{0}+\theta_{0}^{2}.\label{nuova equazione secondo momento}\end{equation}
...
\widehat{\theta}=M_{r}^{-1}(M_{r,n})\simeq M_{r}^{-1}\left(M_{r}(\theta_{0})\right)=\theta_{0}\text{.}\end{equation}
...
\widehat{\theta}=M_{r}^{-1}(M_{r,n})\text{,}\label{approx momento inverso}\end{equation}
...
M_{2,n}\simeq M_{2}\left(\mu_{0},\sigma_{0}^{2}\right)=\sigma_{0}^{2}+\mu_{0}^{2}\end{array}\right.\label{esempio momenti normale 1}\end{equation}
...
otteniamo lo stimatore dei momenti di $\theta_{0}=(\mu_{0},\sigma_{0}^{2})$\begin{eqnarray*}
...
\hline \ensuremath{f(x;\frac{1}{4})} &  \ensuremath{\frac{27}{64}} &  \ensuremath{\frac{27}{64}} &  \ensuremath{\frac{9}{64}} &  \ensuremath{\frac{1}{64}}\\\hline \end{tabular}\]
...
0.25 & \text{quando osserviamo che }x=0,1\\
0.75 & \text{quando osserviamo che }x=2,3\end{array}\right..\]
Lo stimatore {}``decide\textquotedblright{} così il valore di $p$,
notato $\widehat{p}$, per ogni possibile $x$, in modo tale che \[
...
f(6;p)=C_{25,6}\ p^{6}(1-p)^{19}\ \ \ \ \ \text{per }0\leq p\leq1\text{.}\label{stima max likel iniziale}\end{equation}
...
mostra quindi che il valore stimato del parametro sconosciuto massimizza,
per dato valore osservato di $x$, la funzione di proba\-bi\-li\-tà
\foreignlanguage{english}{$f(x;p)$}. La definizione di stimatore
di massima verosimiglianza necessita dapprima la definizione di funzione
di ve\-ro\-si\-mi\-glian\-za.\bigskip{}
...
variabili aleatorie discrete essa corrisponde a una probabilità. Osservati
...
Vale dunque che \begin{equation}
...
che prende il nome di stimatore di massima verosimiglianza.\bigskip{}
...
$L(\theta)$ e della cosiddetta $\log$-verosimiglianza, definita
come $ln(L(\theta))$, sono gli stessi. %
\begin{figure}
\noindent \begin{centering}
...
di verosimiglianza massimizzare la stessa $L(\theta)$ oppure il suo
logaritmo $\ln L(\theta)$. I punti di massimo e minimo delle due
funzioni sono identici (si veda a tal riguardo la Figura \ref{fig:Generica-funzione-g}).
...
\mathcal{P}=\left\{ f(x;p),\ p\in\left[0,1\right]\right\} \text{,}\]
 con \[
f(x;p)=\left\{ \begin{array}{cl}
p & \text{se }x=1,\\
q & \text{se }x=0,\\
0 & \text{altrimenti.}\end{array}\right.\]
 Le realizzazioni campionarie $x_{1},...,x_{n}$ di $X_{1},...,X_{n}$
formeranno una successione di $0$ e di $1.$ Poiché $x_{1}$ sarà
uguale a $0$ o $1$, varrà la seguente uguaglianza \begin{equation}
f(x_{1};p)=p^{x_{1}}q^{1-x_{1}}.\label{densita bernoulli}\end{equation}
 Infatti, se $x_{1}=1$, $f(1;p)=p^{1}q^{1-1}=p$ mentre se $x_{1}=0$,
$f(0;p)=p^{0}q^{1-0}=q$. Poiché le $n$ variabili aleatorie $X_{1},...,X_{n}$
...
il metodo dei momenti ed il metodo di massima verosimiglianza. Ovviamente
...
è la seguente: {}``Dati due stimatori esiste un modo per decidere
quale dei due sia il migliore?\textquotedblright{}. Per decidere questo
è necessario chiarire in che senso uno stimatore può essere migliore
di un altro. A tal fine definiremo alcune proprietà che uno stimatore
può possedere oppure no. Esse ci aiuteranno a decidere se e in che
misura uno stimatore è migliore di un altro.
...
Avremo quindi che lo stimatore $\widehat{\mu}_{5}$, pur essendo uno
stimatore distorto di \foreignlanguage{english}{$\mu$}, è superiore
allo stimatore $\widehat{\mu}_{2}$ in media quadratica per valori
di $\mu$ compresi nell'intervallo $(-\sqrt{2/3},\sqrt{2/3})$ (confronta
Figura \ref{fig:Confronto-EQM-}).%
\begin{figure}
\noindent \begin{centering}
...
di uno stimatore e la sua varianza. Per introdurre e prima di affrontare
questa discussione presentiamo la presente definizione.
...
\sum\limits _{i=1}^{n}\frac{\partial\ln f(x_{i};\theta)}{\partial\theta}\mid_{\theta=\widehat{\theta}_{n}}=0.\]
 Allora vale che 
\begin{enumerate}
...
\sqrt{n}\left(\widehat{\theta}_{n}-\theta\right)\underset{n\rightarrow\infty}{\sim}N(0,\frac{1}{I(\theta)})\]
 dove $I(\theta):=E\left[\left(\frac{\partial\ln f(X;\theta)}{\partial\theta}\right)^{2}\right]$
...
\begin{remark} Per il calcolo di $I(\theta)$ la funzione $\ln f(x;\theta)$
è derivata rispetto a $\theta$ (la $x$ è considerata costante!).
La stessa cosa vale per la derivata seconda $\partial^{2}\ln f(x;\theta)/\partial\theta^{2}$.
\end{remark}
...
\end{enumerate}
\noindent Dunque, come nel caso della media campionaria $\overline{X}_{n}$,
...
del quadrato di $\frac{\partial\ln f(X;\theta)}{\partial\theta}$.
...
\sigma^{2}=E\left[\left(\frac{\partial\ln f(X;\mu)}{\partial\mu}\right)^{2}\right]^{-1}=-\left[E\left(\frac{\partial^{2}\ln f(X;\mu)}{\partial\mu^{2}}\right)\right]^{-1}.\label{verifica uguaglianza fisher}\end{equation}
 \end{esempio}
...
come segue: 
\begin{enumerate}
\item Calcolate la derivata $\frac{\partial\ln f(x;\mu)}{\partial\mu}$,
il suo quadrato ed infine il valore atteso del suo quadrato, notato
$E\left[\left(\frac{\partial\ln f(X;\mu)}{\partial\mu}\right)^{2}\right]$. 
\item Calcolate la derivata seconda $\partial^{2}\ln f(x;\mu)/\partial\mu^{2}$
...
$X$ di densità $f(x;\theta)$ e $T(X_{1},...,X_{n})$ uno stimatore
...
della famiglia parametrica $\mathcal{F}$ di densità che si intende
...
di valore atteso $-0.545$ ${{}^{\circ}}C$ e deviazione standard
$0.008$ ${{}^{\circ}}C$. La famiglia parametrica di densità è ancora
Normale \[
...
il valore di $\mu$ sarebbe conosciuto ed uguale a quello calcolato
dal Laboratorio cantonale. Sia la distribuzione di $X$ che la distribuzione
campionaria $f(x_{1},...,x_{n})$ sarebbero conosciuti. Per contro,
qualora il latte venisse diluito con acqua, la sua temperatura di
gelo sarebbe superiore a quella del latte {}``puro\textquotedblright{}.
Avremmo quindi%
...
ad un unico valore anziché all'intero campione. Nel nostro caso ad
esempio, calcoliamo la media delle cinque osservazioni, $\overline{x}=-0.536$.
Grazie alle proprietà della distribuzione Normale sappiamo che $\overline{X}\sim N(\mu,\frac{0.008^{2}}{5})$.
Sotto l'ipotesi $H_{0}$ la distribuzione di $\overline{X}$ è dunque
$N(-0.545,\frac{0.008^{2}}{5})$. 
...
dal caseificio (vedi Figura \ref{fig:latte P-valore}):\begin{eqnarray*}
...
di osservare un valore della statistica $S$ almeno estremo quanto
il valore osservato è detta $p$-value. \end{definizione}
...
del termine {}``più estremo''? Con {}``più estremo'' si intende
{}``più lontano'', {}``più distante'' - rispetto al valore atteso
$\mu$ sotto $H_{0}$ - in direzione dell'ipotesi alternativa. Il
...
$H_{0}$ dipenderà dalla {}``quantità\textquotedblright{} minima
di evidenza sotto la quale non siamo più disposti a mantenere $H_{0}$
...
\noindent Il $p$-value è dunque uguale a\begin{eqnarray*}
P_{H_{0}}(\overline{X}\leq0.5985,\:\overline{X}\geqslant0.6015) & = & P_{H_{0}}(|\overline{X}-0.6|\geq0.0015)\\
 & = & P_{H_{0}}(\frac{|\overline{X}-0.6|}{0.001}\geq1.5)=13.4\%.\end{eqnarray*}
...
nulla è qualcosa del tipo {}``... è uguale a ...'' oppure {}``...
nessun problema ...\textquotedblright{} oppure {}``... nessun effetto
...\textquotedblright{} oppure {}``... nessuna differenza ...\textquotedblright{}. 
...
seguente uguaglianza :\begin{equation}
P_{H_{0}}(\overline{X}\geq k_{5\%})=0.05.\label{k alpha media}\end{equation}
...
P\left(Z\leq z_{0.95}\right) & = & 0.95\end{eqnarray*}
...
\overline{x}\geq k_{5\%}=-0.539\label{disequazione test destro}\end{equation}
...
$R\subset\Omega^{n}$ delle realizzazioni $(x_{1},...,x_{n})$ di
$(X_{1},...,X_{n})$ per cui $S\geq k_{\alpha}$ (e quindi tali per
cui il test è rifiutato) è chiamato la regione critica del test o
semplicemente regione critica\[
...
\noindent Ma cos'è la regione critica? Diciamo che tendenzialmente
rigetteremo l'ipotesi nulla se osserveremo dei valori della statistica
estremi in direzione dell'ipotesi alternativa. Essendo questo un test
bilaterale (il valore atteso del diametro delle sfere potrebbe essere
maggiore o minore rispetto al valore indicato nell'ipotesi nulla),
dobbiamo considerare valori estremi della statistica in ambo le direzioni.
Ciò è possibile assegnando metà del livello di si\-gni\-fi\-ca\-ti\-vi\-tà
al lato sinistro della distribuzione e l'altra metà al lato destro.
Dovremo quindi calcolare due quantili della distribuzione della statistica
$S$, $\overline{X}$ in questo caso. Il primo (lato sinistro) è il
$2.5\%$-quantile (in generale sarà l'$\frac{\alpha}{2}$-quantile)
ed il secondo (lato destro) è il $97.5\%-$quantile ($(1-\frac{\alpha}{2})$-quantile):\begin{equation}
\left\{ \begin{array}{l}
P_{H_{0}}(\overline{X}\leq k_{s,5\%})=2.5\%\\
P_{H_{0}}(\overline{X}\geq k_{d,5\%})=2.5\%\end{array}\right.\end{equation}


\[
\left\{ \begin{array}{l}
P_{H_{0}}\left(\overline{X}-0.6\leq k_{s,5\%}-0.6\right)=2.5\%\\
P_{H_{0}}\left(\overline{X}-0.6\geq k_{d,5\%}-0.6\right)=2.5\%\end{array}\right.\]
 \[
\left\{ \begin{array}{l}
P_{H_{0}}(\frac{\overline{X}-0.6}{0.001}\leq\frac{k_{s,5\%}-0.6}{0.001})=2.5\%\\
P_{H_{0}}(\frac{\overline{X}-0.6}{0.001}\geq\frac{k_{d,5\%}-0.6}{0.001})=2.5\%\end{array}\right.\]
...
P(Z\geq z_{0.975})=2.5\%\end{array}\right.\Leftrightarrow P(|\frac{\overline{X}-0.6}{0.001}|\geq z_{0.975})=0.05\]
...
ovvero se \begin{equation}
\overline{x}\leq k_{s,5\%}=0.598\text{ oppure }\overline{x}\geq k_{d,5\%}=0.6019\label{disequazione test bilaterale}\end{equation}
...
$R$ delle possibili realizzazioni $(x_{1},...,x_{n})$ di $(X_{1},...,X_{n})$
per cui $S(x_{1},...,x_{n})\leq k_{s,\alpha}$ oppure $S(x_{1},...,x_{n})\geq k_{d,\alpha}$
...
\hspace{-1.3cm}\includegraphics{codiceR/grafici/testBilaterale}
\par\end{raggedright}
...
delle possibili realizzazioni campionarie del nostro esperimento statisico:
$R\subset\Omega^{n}$. Se i valori $(x_{1},...,x_{n})$ che osserveremo
...
\begin{equation}
P_{H_{0}}(S\in E')=P_{H_{0}}(S^{-1}(E'))=P_{H_{0}}(R)=\alpha\text{.}\end{equation}
 \end{remark}
...
un'altra statistica? Vogliamo considerare questa possibilità per l'Esempio
\ref{esempio caseificio p1} e la statistica $S=\min(X_{1},...,X_{5})$.
Supponiamo che i cinque valori osservati siano uguali a%
...
statistica $S$ e la funzione di ripartizione (densità) delle V.A.
$X_{i}$, aggiungeremo la lettera $S$ o $X_{i}$ al piede di $F$
($f$).%
...
densità di $S$. Derivando entrambi i lati rispetto ad $s$ otteniamo\begin{equation}
...
Grazie al paragrafo \ref{distribuzione del minimo} conosciamo la
distribuzione di $S$. Il nuovo punto $4$. diventa

\item Abbiamo deciso di utilizzare $S=min(X_{1},\ldots,X_{5})$ quale statistica
per costruire il test. Sotto l'\emph{ipotesi nulla} ogni singola $X_{i}\sim N(-0.545,0.008^{2}).$
...
In questo caso il $p$-value è da calcolarsi rispetto alla nuova statistica
$S=min(X_{1},\ldots,X_{5}).$ Come detto in precedenza, il suo valore
realizzato è $s=-0.542$. Il $p$-value sarà dunque\begin{eqnarray*}
...
notato $k_{\alpha}$.\begin{eqnarray}
...
\item [7.] Per valori della statistica $S$ superiori a $k_{5\%}=-0.546$
(estremi in direzione dell'ipotesi alternativa) rigetteremo l'ipotesi
nulla. In questo caso la regione critica $R$ è definita dall'insieme
delle possibili realizzazioni $(x_{1},...,x_{5})$ tali che $\min(x_{1},...,x_{5})\geq k_{5\%}=-0.546$,
\[
...
\begin{itemize}
\item \noindent test bilaterale\begin{eqnarray*}
R & = & \left\{ (x_{1},...,x_{n})\mid S(x_{1},...,x_{n})\leq k_{s,\alpha}\text{{\ oppure\ }}S(x_{1},...,x_{n})\geq k_{d,\alpha}\right\} .\end{eqnarray*}
...
\alpha & = & P_{H_{0}}(\text{rifiutare }H_{0})=P_{H_{0}}(\overline{X}\geq k_{\alpha})\\
 ...
 & = & P\left(\underset{\sim N(0,1)}{Z}\geq\frac{k_{\alpha}+0.545}{0.008/\sqrt{5}}\right)\end{eqnarray*}
...
k_{\alpha} & = & -0.545+z_{1-\alpha}0.008/\sqrt{5}\end{eqnarray*}
...
probabilità dell'evento $X\geq1.645$ e la sua probabilità è semplicemente\begin{equation}
P\left(X\geq1.645\right).\label{Prob generica}\end{equation}
...
Calcolando abbiamo\begin{eqnarray*}
\beta & = & P_{H_{A}}(\overline{X}\geq-0.539)=P_{H_{A}}\left(\underset{Z\sim N(0,1)}{\underbrace{\frac{\overline{X}-(-0.54)}{0.008/\sqrt{5}}}}\geq\frac{-0.539-(-0.54)}{0.008/\sqrt{5}}\right)\\
 & = & P\left(Z\geq0.2795\right)=1-P\left(Z\leq0.2795\right)=0.39\end{eqnarray*}
...
in entrambi i casi, la probabilità di esse\noindentre ingiustamente condannato
...
R=\left\{ (x_{1},...,x_{n})\mid S(x_{1},...,x_{n})\geq k_{\alpha}\right\} \end{equation}
...
} il vincolo\begin{equation}
P_{H_{0}}(R)=P_{H_{0}}(S\geq k_{\alpha})=\alpha\text{.}\label{soglia k_alpha}\end{equation}
...
\selectlanguage{italian}%
\noindent e\begin{equation}
P_{H_{A}}(R^{\ast})=P_{H_{A}}(S^{\ast}\geq k_{\alpha}^{\ast})=P(\text{rifiuto }H_{0}\text{ }|\ H_{0}\text{ è falsa})=\beta\end{equation}
...
P_{r}(R^{\ast})-P_{r}(R) & \geq & k_{\alpha}^{\ast}\sum\limits _{g\in A}p_{s}(g)-\sum\limits _{g\in C}p_{r}(g)\\
 ...
 Infine, notiamo che la quantità fra parentesi è uguale 0. Infatti\begin{eqnarray*}
...
R^{\ast}=\left\{ (x_{1},...,x_{n})\ |\ S^{\ast}(x_{1},...,x_{n})=\frac{f(x_{1},...,x_{n};\theta_{A})}{f(x_{1},...,x_{n};\theta_{0})}\geq k_{\alpha}^{\ast}\right\} .\label{criterio neyman pearson}\end{equation}
...
R=\left\{ (x_{1},...,x_{n})\ |\ \exp\left(-\frac{1}{2\sigma^{2}}\left[n(\mu_{1}^{2}-\mu_{0}^{2})-2n\left(\mu_{1}-\mu_{0}\right)\overline{x}\right]\right)\geq k_{\alpha}\right\} ,\label{a1}\end{equation}
...
R & = & \left\{ (x_{1},...,x_{n})\ |\ -\frac{1}{2\sigma^{2}}\left[n(\mu_{1}^{2}-\mu_{0}^{2})-2n\left(\mu_{1}-\mu_{0}\right)\overline{x}\right]\geq\ln(k_{\alpha})\right\} \\
 ...
 & = & \left\{ (x_{1},...,x_{n})\ |\ \overline{x}\geq\widetilde{k}_{\alpha}\right\} .\end{eqnarray*}
...
otteniamo\begin{equation}
P\left(Z\leq\frac{k_{\alpha}-\mu_{0}}{\sigma/\sqrt{n}}\right)=1-\alpha\text{.}\label{uguaglianza quantile standard}\end{equation}
...
k_{\alpha} & = & \mu_{0}+z_{1-\alpha}\frac{\sigma}{\sqrt{n}}\text{.}\label{valore critico test sulla media normale}\end{eqnarray}
...
\beta & = & P_{H_{A}}(\overline{X}\geq k_{\alpha})\\
 ...
 & = & 1-P\left(Z\leq\frac{k_{\alpha}-\mu_{1}}{\sigma/\sqrt{n}}\right)\end{eqnarray*}
...
ci permette di determinare la numerosità del campione necessario \begin{eqnarray*}
...
$k_{\alpha}$\begin{eqnarray*}
P_{H_{0}}(\overline{X} & \geq & k_{0.05})=0.05.\end{eqnarray*}
...
$\alpha$ il valore critico $k_{\alpha}$:\begin{equation}
P_{H_{0}}\left(\underset{Z\sim N(0,1)}{\underbrace{\frac{\overline{X}-\mu_{0}}{\sigma/\sqrt{n}}}}\geq\frac{k_{\alpha}-\mu_{0}}{\sigma/\sqrt{n}}\right)=\alpha.\label{rapporto t1}\end{equation}
...
P_{H_{0}}\left(\underset{\sim\ ?}{\underbrace{\frac{\frac{\overline{X}-\mu_{0}}{\sigma/\sqrt{n}}}{\widehat{\sigma}/\sigma}}}\geq\frac{k_{\alpha}-\mu_{0}}{\widehat{\sigma}/\sqrt{n}}\right)=\alpha.\label{rapporto t2}\end{equation}
...
P\left(t_{n-1}\geq\frac{k_{\alpha}-\mu_{0}}{\widehat{\sigma}/\sqrt{n}}\right) & = & \alpha\end{eqnarray*}
...
P\left(t_{n-1}\leq\frac{k_{\alpha}-\mu_{0}}{\widehat{\sigma}/\sqrt{n}}\right) & = & 1-\alpha.\end{eqnarray*}
...
\noindent In questo caso avremo \begin{eqnarray*}
...
e quelle teoriche, debitamente {}``standardizzate''. Al fine di
eseguire correttamente il test occorre ancora una volta conoscere
la distribuzione della statistica $S=\sum_{k=1}^{K}\frac{\left({\small O}_{k}-{\small E}_{k}\right)^{2}}{{\small E}_{k}}$.
